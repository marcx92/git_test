...
SELECT origin, f.Year, f.Month, AVG(arrdelay) FROM flights as f GROUP BY origin, f.Year, f.Month;
...
SELECT c.Description, SUM(f.Cancelled) FROM flights as f LEFT JOIN carriers as c on c.Code = f.UniqueCarrier GROUP BY c.Description ORDER BY SUM(f.Cancelled) DESC;
 
/*6. L’identificador dels 10 avions que més distància han recorregut fent vols. */
 
 SELECT FlightNum, SUM(Distance) FROM flights GROUP BY FlightNum LIMIT 10;
